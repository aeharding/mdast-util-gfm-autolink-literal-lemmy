...
function buildTransformGfmAutolinkLiterals(connectedInstance) {
  const findCommunity = buildFindCommunity(connectedInstance)
 ...
 * @param {string} connectedInstance
 */
function buildFindCommunity(connectedInstance) {
  /**
   * @type {ReplaceFunction}
   * @param {string} _
   * @param {string} atext
  ...
  function findCommunity(_, atext, label, match) {
    ...
    const communityName = atext.slice(1)
    const server = label
    ...
    return {
      ...
      children: [{type: 'text', value: `!${communityName}@${server}`}]
    }
  }
 ...
 * @param {string} connectedInstance
 */
function buildFindUser(connectedInstance) {
  /**
   * @type {ReplaceFunction}
   * @param {string} _
   * @param {string} atext
  ...
  function findUser(_, atext, label, match) {
    ...
    return {
      ...
      children: [{type: 'text', value: `@${userName}@${server}`}]
    }
  }
